...
const Home = props => {
    return (
        <ScrollView style={styles.homeContainer}>
            ...
            >
                {dummy.map((item, idx) => <Card key={idx} title={item.title} image={item.img}></Card>)}
            </ScrollView>
            <Text style={{ color: "#747882", padding: 10, paddingBottom: 0, fontSize: 24, fontWeight: "bold" }}>Your Articles</Text>
                ...
                renderItem={article => {
                    ...
                    return (
                        <View style={{ borderColor: 'black', borderBottomWidth: 1, paddingTop: 10, paddingBottom: 5 }}>
                            <ArticleCard image={article.item.img} text={article.item.title}></ArticleCard>
                        </View>
                    )
                }}
            >
